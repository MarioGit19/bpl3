...
export class Lexer {
    ...
    public scanTokens(): Token[] {
        ...
        this.tokens.push(new Token(TokenType.EOF, "", null, this.line, this.column, this.filePath));
        return this.tokens;
    }
    ...
    private scanToken(): void {
        ...
        switch (c) {
            ...
            case '/':
                ...
                break;
            case '%':
                this.addToken(this.match('=') ? TokenType.PercentEqual : TokenType.Percent);
                break;
            case '&':
                if (this.match('=')) {
            ...
            default:
                if (this.isDigit(c)) {
                    ...
                    console.error(`Unexpected character: ${c} at line ${this.line}, col ${this.column}`);
                }
                break;
        }
    }
    ...
    private number(): void {
        ...
        if (this.peekPrevious() === '0' && this.peek() === 'x') {
            ...
            this.addToken(TokenType.NumberLiteral, this.source.substring(this.start, this.current));
            return;
        }
        // Binary
        ...
        this.addToken(TokenType.NumberLiteral, parseFloat(this.source.substring(this.start, this.current).replace(/_/g, '')));
    }
    ...
    private addToken(type: TokenType, literal: any = null): void {
        const text = this.source.substring(this.start, this.current);
        this.tokens.push(new Token(type, text, literal, this.line, this.startColumn, this.filePath));
    }
}
