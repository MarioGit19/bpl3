...
export class CodeGenerator {
    ...
    private collectStructLayouts(program: AST.Program) {
        for (const stmt of program.statements) {
            if (stmt.kind === "StructDecl") {
                ...
                const fields = decl.members.filter(m => m.kind === "StructField") as AST.StructField[];
    ...
    private generateExtern(decl: AST.ExternDecl) {
        ...
        const retType = decl.returnType ? this.resolveType(decl.returnType) : "void";
    ...
    private generateFunction(decl: AST.FunctionDecl) {
        ...
        const lastLine = this.output.length > 0 ? this.output[this.output.length - 1]! : "";
    ...
    private generateVariableDecl(decl: AST.VariableDecl) {
        ...
        const type = decl.typeAnnotation ? this.resolveType(decl.typeAnnotation) : this.resolveType(decl.initializer!.resolvedType!);
    ...
    private generateBinary(expr: AST.BinaryExpr): string {
        ...
        if (leftType.endsWith("*") && rightType === "i64") {
            ...
            if (expr.operator.type === TokenType.Plus) {
                this.emit(`  ${reg} = getelementptr inbounds ${leftType.slice(0, -1)}, ${leftType} ${left}, i64 ${right}`);
                return reg;
            } else if (expr.operator.type === TokenType.Minus) {
                // ptr - int -> ptr + (-int)
                const negRight = this.newRegister();
                this.emit(`  ${negRight} = sub i64 0, ${right}`);
                this.emit(`  ${reg} = getelementptr inbounds ${leftType.slice(0, -1)}, ${leftType} ${left}, i64 ${negRight}`);
                return reg;
            }
        }
        
        // int + ptr (commutative)
        if (leftType === "i64" && rightType.endsWith("*") && expr.operator.type === TokenType.Plus) {
             this.emit(`  ${reg} = getelementptr inbounds ${rightType.slice(0, -1)}, ${rightType} ${right}, i64 ${left}`);
             return reg;
        }
        ...
        switch (expr.operator.type) {
            ...
            case TokenType.Slash: op = "sdiv"; break; // Signed division
            case TokenType.EqualEqual: op = "icmp eq"; break;
            case TokenType.BangEqual: op = "icmp ne"; break;
            case TokenType.Less: op = "icmp slt"; break;
            case TokenType.LessEqual: op = "icmp sle"; break;
            case TokenType.Greater: op = "icmp sgt"; break;
            case TokenType.GreaterEqual: op = "icmp sge"; break;
        }
    ...
    private generateAddress(expr: AST.Expression): string {
        if (expr.kind === "Identifier") {
            ...
            if (fieldIndex === undefined) {
                throw new Error(`Unknown field '${memberExpr.property}' in struct '${structName}'`);
            }
            
            const addr = this.newRegister();
            const llvmType = `%struct.${structName}`;
            this.emit(`  ${addr} = getelementptr inbounds ${llvmType}, ${llvmType}* ${objectAddr}, i32 0, i32 ${fieldIndex}`);
            return addr;
            ...
            if (objType.arrayDimensions.length > 0) {
                ...
                if (llvmType.startsWith("[")) {
                    this.emit(`  ${addr} = getelementptr inbounds ${llvmType}, ${llvmType}* ${objectAddr}, i64 0, i64 ${indexVal}`);
                } else {
                    // Pointer
                    this.emit(`  ${addr} = getelementptr inbounds ${llvmType}, ${llvmType}* ${objectAddr}, i64 ${indexVal}`);
                }
                ...
                this.emit(`  ${addr} = getelementptr inbounds ${elemType}, ${ptrType} ${ptrReg}, i64 ${indexVal}`);
            } else {
    ...
    private generateStructLiteral(expr: AST.StructLiteralExpr): string {
        ...
        for (const field of expr.fields) {
            ...
            if (currentReg === "undef") {
                this.emit(`  ${nextReg} = insertvalue ${llvmType} undef, ${fieldType} ${fieldValue}, ${fieldIndex}`);
            } else {
                this.emit(`  ${nextReg} = insertvalue ${llvmType} ${currentReg}, ${fieldType} ${fieldValue}, ${fieldIndex}`);
            }
            currentReg = nextReg;
        }
    ...
    private resolveType(type: AST.TypeNode): string {
        ...
        if (type.kind === "BasicType") {
            let llvmType = "";
            switch (type.name) {
                case "int": llvmType = "i64"; break;
                case "float": llvmType = "double"; break;
                case "bool": llvmType = "i1"; break;
                case "char": llvmType = "i8"; break;
                case "void": llvmType = "void"; break;
                case "string": llvmType = "i8*"; break;
                default: llvmType = `%struct.${type.name}`; break;
            }
    ...
    private isTerminator(line: string): boolean {
        ...
        return trimmed.startsWith("ret") || trimmed.startsWith("br") || trimmed.startsWith("unreachable");
    }
    ...
    private generateGlobalVariable(decl: AST.VariableDecl) {
        ...
        const type = this.resolveType(decl.typeAnnotation || decl.initializer!.resolvedType!);
        ...
        if (decl.initializer) {
            ...
            if (type === "i64" || type === "i32" || type === "i8" || type === "i1") init = "0";
    ...
    private generateSizeof(expr: AST.SizeofExpr): string {
        ...
        const ptrReg = this.newRegister();
        this.emit(`  ${ptrReg} = getelementptr ${llvmType}, ${llvmType}* null, i32 1`);
