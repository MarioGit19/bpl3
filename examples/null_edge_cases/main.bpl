...
frame testNestedAccess() {
    ...
    local val: int = outer.inner.value; # Should trap on outer.inner
...
frame testArrayInStruct() {
    ...
    local val: int = c.items[5]; # Should trap on c.items
    printf("Should not reach here\n");
}
...
frame testNullInLoop() {
    ...
    loop (i < 5) {
        local val: int = outer.data; # Should trap on first iteration
        i = i + 1;
    }
    printf("Should not reach here\n");
}
...
frame testStructArrayElement() {
    ...
    local arr: Outer[3];
    arr[1] = null;
    local val: int = arr[1].data; # Should trap
    printf("Should not reach here\n");
}
...
frame testDeepNested() {
    ...
    local val: int = outer.inner.value;
    printf("Should not reach here\n");
}
