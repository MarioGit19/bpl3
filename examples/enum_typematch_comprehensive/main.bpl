...
frame process_message(msg: Message) ret int {
...
frame handle_result(r: Result<int, string>) ret int {
...
frame analyze_message(msg: Message) ret string {
...
frame message_category(msg: Message) ret string {
    if (match<Message.Empty>(msg)) {
        return "empty";
...
frame validate_result(r: Result<int, string>) ret int {
    if (match<Result.Ok>(r)) {
...
frame main() ret int {
    ...
    printf("\n=== Result Validation ===\n");
    local v1: int = validate_result(r1);
    local v2: int = validate_result(r2);
    printf("Results: %d, %d\n", v1, v2);
