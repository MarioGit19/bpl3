...
export class PackageManager {
  ...
  loadManifest(packageDir: string): PackageManifest {
    ...
    try {
      ...
      if (!/^[a-z0-9-]+$/.test(manifest.name)) {
        throw new Error(`Invalid package name: ${manifest.name} (use lowercase and hyphens only)`);
      }
  ...
  private getAllBplFiles(dir: string): string[] {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (item !== "node_modules" && item !== "bpl_modules" && !item.startsWith(".")) {
  ...
  install(
    ...
    const targetDir = options.global ? this.globalPackageDir : this.localPackageDir;
    ...
    if (fs.existsSync(packageSource)) {
      ...
      const matching = files.filter(f => f.startsWith(packageSource) && f.endsWith(".tgz"));
      ...
      matching.sort();
      tarballPath = path.join(this.globalPackageDir, matching[matching.length - 1]!);
    }
  ...
  list(options: { global?: boolean } = {}): PackageInfo[] {
    const searchDir = options.global ? this.globalPackageDir : this.localPackageDir;
  ...
  private getPackageEntryPoint(packagePath: string): string | null {
    ...
    try {
      ...
      for (const ext of [".bpl", ".x", ""]) {
        const fullPath = entryPath.endsWith(".bpl") || entryPath.endsWith(".x") 
          ? entryPath 
          : entryPath + ext;
