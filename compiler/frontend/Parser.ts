...
export class Parser {
  ...
  private functionDeclaration(): AST.FunctionDecl {
    ...
    if (this.match(TokenType.Less)) {
      do {
        genericParams.push(
          this.consume(TokenType.Identifier, "Expected generic parameter name.")
            .lexeme,
        );
      } while (this.match(TokenType.Comma));
  ...
  private block(): AST.BlockStmt {
    const startToken = this.check(TokenType.LeftBrace)
  ...
  private or(): AST.Expression {
    ...
    while (this.match(TokenType.OrOr)) {
      ...
      expr = {
        ...
        operator,
        right,
        location: this.loc(expr.location, right.location),
      };
    }
    return expr;
  }
  ...
  private unary(): AST.Expression {
    if (
      ...
      return {
        ...
        operand: right,
        isPrefix: true,
        location: this.loc(operator, right.location),
      };
    }
    return this.postfix();
  }
  ...
  private primary(): AST.Expression {
    ...
    if (this.match(TokenType.Nullptr))
      return {
        kind: "Literal",
        value: null,
        raw: "nullptr",
    ...
    if (this.match(TokenType.Cast)) {
      ...
      return {
        ...
        targetType: type,
        expression: expr,
        location: this.loc(start, this.previous()),
      };
    }
