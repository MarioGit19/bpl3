...
- Generate completion scripts (bash/zsh) either via commander helpers or custom templates
- Bundle scripts with releases and document installation steps (source file, or place into standard completion directories)
- Consider dynamic completion for file paths, targets, and common flags where feasible
- Add a CLI subcommand (e.g., `bpl completion`) to print/install completions

**Acceptance Criteria:**

- Users can enable completions in bash and zsh following documented steps
- Commands and flags complete correctly; file/target suggestions work where implemented
- Completion scripts are shipped or generated by the CLI command
